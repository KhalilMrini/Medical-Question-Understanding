...
from fairseq.models.transformer import TransformerModel, TransformerGradSoftModel
...
@register_model('bart_gradsoft')
class BARTGradSoftModel(TransformerGradSoftModel):
    ...
    def __init__(self, args, encoder, decoder1, decoder2):
        super().__init__(args, encoder, decoder1, decoder2)
    ...
    @staticmethod
    def add_args(parser):
        super(BARTGradSoftModel, BARTGradSoftModel).add_args(parser)
        parser.add_argument(
            '--pooler-dropout', type=float, metavar='D',
            help='dropout probability in the masked_lm pooler layers'
        )
    ...
    def forward(
        self, src_tokens, src_lengths, prev_output_tokens,
        features_only=False, features_only_2=False,
        classification_head_name=None, 
        classification_head_name_2=None, **kwargs
    ):
        if classification_head_name is not None:
            features_only = True

        if classification_head_name_2 is not None:
            features_only_2 = True
        ...
        x2, extra2 = self.decoder2(
            prev_output_tokens,
            encoder_out=encoder_out,
            features_only=features_only_2,
            **kwargs,
        )

        if classification_head_name is not None:
            sentence_representation1 = x1[
                src_tokens.eq(self.encoder.dictionary.eos()), :
            ].view(x1.size(0), -1, x1.size(-1))[:, -1, :]
            x1 = self.classification_heads[classification_head_name](
                sentence_representation1
            )

        if classification_head_name_2 is not None:
            sentence_representation2 = x2[
                src_tokens.eq(self.encoder.dictionary.eos()), :
            ].view(x2.size(0), -1, x2.size(-1))[:, -1, :]
            x2 = self.classification_heads[classification_head_name_2](
                sentence_representation2
            )
...
@register_model_architecture('bart_gradsoft', 'bart_large_gradsoft')
def bart_large_gradsoft_architecture(args):
    args.encoder_embed_path = getattr(args, 'encoder_embed_path', None)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 1024)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 4*1024)
    args.encoder_layers = getattr(args, 'encoder_layers', 12)
